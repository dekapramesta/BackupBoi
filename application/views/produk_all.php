	...
	<div class="container">
		<div class="destination-grid-content">
			<div class="row">
				...
				<div class="col-md-6" style="margin-top:10px;">
					<select id="kota" disabled class="form-control">
					...
					if ($this->session->userdata('current_language') == 'english') {
						...
						foreach ($produk as $m) {
							...
							if (strpos($m["nama_produk"], '-') !== false) {
								$wisataNama = str_replace('-', '_', $m["nama_produk"]);
							} else {
								$wisataNama = $m["nama_produk"];
							}
							$link = base_url() . 'Produk/detail/' . str_replace(' ', '-', $m["nama_provinsi"]) . '/' . str_replace(' ', '-', $m["nama_kota"]) . '/' . str_replace(' ', '-', $wisataNama);
							$wisatae = @$wisatae . "
									<div class='awe-masonry__item'>
										<a href=" . $link . ">
											<div class='image-wrap image-cover'>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#provinsi').change(function() {
			...
			$(location).attr('href', '<?= base_url("Produk/Semua-Jenis/") ?>' + str);
		...
		$('#kota').change(function() {
			...
			$(location).attr('href', '<?= base_url("Produk/Semua-Jenis/") ?>' + provinsi + '/' + city);
		});
