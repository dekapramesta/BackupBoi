 <?php $title = "<i class='fa fa-binoculars'></i>&nbsp;Fasilitas Pendukung"; ?>
...
<div class="widget-box">
<div class="widget-header">
		<h4 class="widget-title">Form Fasilitas Pendukung</h4>
...
<form id="form-add" class="form-horizontal" action="<?= site_url('Fasilitas_pendukung/ajax_add')?>" method="POST" role="form" enctype="multipart/form-data">
	...
	<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Fasilitas Pendukung </label>
		<div class="col-sm-10">
			<input type="text" id="faspen_nama" name="faspen_nama" placeholder="Fasilitas Pendukung" class="col-xs-10 col-sm-5" />
...
<div class="tabbable">
	...
	<form id="form-upload" class="form-horizontal" role="form" action="<?= site_url('Fasilitas_pendukung/upload')?>" method="POST" enctype="multipart/form-data">
		<input type="hidden" name="faspen_id"/> 
		<div class="form-body">
			<div class="form-group" >
...
<script>
	var zonk='';
	var save_method;
	var link = "<?php echo site_url('Fasilitas_pendukung')?>";
	var table;
	...
	function save() {
		var url;
		url = "<?= site_url()?>Fasilitas_pendukung/update/";
		tinyMCE.triggerSave();
	...
	function fasilitas_pendukung(){
        ...
        "ajax": {
            "url": "<?php echo site_url('Fasilitas_pendukung/ajax_list')?>",
            "type": "POST"
        },
	...
	function fasilitas_pendukung_en(){
		table = $('#dynamic-table').DataTable({ 
        ...
        // Load data for the table's content from an Ajax source
        "ajax": {
            "url": "<?php echo site_url('Fasilitas_pendukung/ajax_list_en')?>",
            "type": "POST"
        },
	...
	function update(id) {
			...
			$.ajax({
				...
				var img = '<?= base_url(); ?>../uploads/fasilitas_wisata/'+result.faspen_icon;
				$('[name="faspen_id"]').val(result.faspen_id);
				$('[name="faspen_nama"]').val(result.faspen_nama);
	...
	function hapus(id) {
		if (confirm('Are you sure delete this data?')) {
			$.ajax ({
				url : "<?php echo site_url('Fasilitas_pendukung/ajax_delete')?>/"+id,
				type: "POST",
				dataType: "JSON",
				success: function(data) {
					setTimeout(function(){
