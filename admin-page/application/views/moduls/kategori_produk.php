<?php $title = "<i class='fa fa-binoculars'></i>&nbsp;Kategori Oleh-oleh"; ?>
...
<form id="form-add" class="form-horizontal" action="<?= site_url('Kategori_produk/ajax_add')?>" method="POST" role="form" enctype="multipart/form-data">
	<input type="hidden" name="id_kategori_produk">
	<div class="form-group">
	<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Kategori Nama </label>
		<div class="col-sm-10">
			<input type="text" id="nama_kategori" name="nama_kategori" placeholder="Nama Kategori" class="col-xs-10 col-sm-5" required oninvalid="this.setCustomValidity('Field kategori harus diisi')" oninput="this.setCustomValidity('')"/>
...
<div class="tabbable">
	...
	<form id="form-upload" class="form-horizontal" role="form" action="<?= site_url('Kategori_produk/upload')?>" method="POST" enctype="multipart/form-data">
		<input type="hidden" name="id_kategori_produk"/> 
		<div class="form-body">
			<div class="form-group" >
...
<script>
	...
	function save() {
		var url;
		url = "<?= site_url()?>Kategori_produk/update/";
		tinyMCE.triggerSave();
	...
	function kategori(){
        ...
        "ajax": {
            "url": "<?php echo site_url('Kategori_produk/ajax_list')?>",
            "type": "POST"
        },
	...
	function kategori_en(){
        ...
        // Load data for the table's content from an Ajax source
        "ajax": {
            "url": "<?php echo site_url('Kategori_produk/ajax_list_en')?>",
            "type": "POST"
        },
	...
	function update(id) {
			...
			$.ajax({
				...
				$('[name="id_kategori_produk"]').val(result.id_kategori_produk);
				$('[name="nama_kategori"]').val(result.nama_kategori);
	...
	function hapus(id) {
		if (confirm('Are you sure delete this data?')) {
			$.ajax ({
				url : "<?php echo site_url('Kategori_produk/ajax_delete')?>/"+id,
				type: "POST",
				dataType: "JSON",
				success: function(data) {
					setTimeout(function(){
