<?php $title = "<i class='fa fa-automobile'></i>&nbsp;Wahana Wisata"; ?>
...
<div class="row">
	<div class="col-xs-12">
		<div id="form-data" style="display:none;">
			<div class="widget-box">
				<div class="widget-header">
					<h4 class="widget-title">Form Wahana</h4>
					...
					<div class="widget-main">
						<div class="row">
							<div class="col-xs-12">
								...
								<form id="form-add" class="form-horizontal" action="<?= site_url('Wahana/ajax_add') ?>" method="POST" role="form" enctype="multipart/form-data">
									<!-- <form class="form-horizontal" action="<?= site_url('Wahana/ajax_add') ?>" method="POST" role="form" enctype="multipart/form-data"> -->
									<input type="hidden" name="wahana_id">
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Nama Wahana </label>
										<div class="col-sm-10">
											<input type="text" id="wahana_nama" name="wahana_nama" placeholder="Nama Wahana" class="col-xs-10 col-sm-5" required oninvalid="this.setCustomValidity('Field nama wahana harus diisi')" oninput="this.setCustomValidity('')" />
									...
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Wahana Deskripsi </label>
										<div class="col-sm-6">
											<textarea class="form-control" id="wahana_deskripsi" name="wahana_deskripsi" placeholder="Wahana Deskripsi" class="col-xs-10 col-sm-5" cols="30" rows="10"></textarea>
											<span class="help-block"></span>
										</div>
									</div>
		...
		<div class="tabbable">
			...
			<div class="tab-content">
				<div id="home" class="tab-pane fade in active">
					<form action="#" id="form" name="form" class="form-horizontal" enctype="multipart/form-data">
						...
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Wahana Deskripsi </label>
							<div class="col-sm-6">
								<textarea class="form-control" id="teks" name="wahana_deskripsi" placeholder="Wahana Deskripsi" class="col-xs-10 col-sm-5" cols="30" rows="10"></textarea>
								<span class="help-block"></span>
							</div>
							<br /><br /><br /><br />
							<div class="col-md-offset-2 col-md-9">
				...
				<div id="messages" class="tab-pane fade">
					<form id="form-upload" class="form-horizontal" role="form" action="<?= site_url('Wahana/upload') ?>" method="POST" enctype="multipart/form-data">
						<input type="hidden" name="wahana_id" />
						<div class="form-body">
							<div class="form-group">
	...
	<script>
		var zonk = '';
		var save_method;
		var link = "<?php echo site_url('Wahana') ?>";
		var table;
		...
		function wahana() {
			table = $('#dynamic-table').DataTable({
				...
				"ajax": {
					"url": "<?php echo site_url('Wahana/ajax_list') ?>",
					"type": "POST"
				},
		...
		function wahana_en() {
			table = $('#dynamic-table').DataTable({
				...
				// Load data for the table's content from an Ajax source
				"ajax": {
					"url": "<?php echo site_url('Wahana/ajax_list_en') ?>",
					"type": "POST"
				},
		...
		function update(id) {
			...
			$.ajax({
				...
				success: function(result) {
					var img = '<?= base_url(); ?>../uploads/wahana/' + result.wahana_icon;
					$('[name="wahana_id"]').val(result.wahana_id);
					$('[name="wahana_nama"]').val(result.wahana_nama);
					$('#preview-upload').attr('src', img);
					tinymce.editors[1].setContent(result.wahana_deskripsi);
		...
		function hapus(id) {
			if (confirm('Are you sure delete this data?')) {
				$.ajax({
					url: "<?php echo site_url('Wahana/ajax_delete') ?>/" + id,
					type: "POST",
					dataType: "JSON",
					success: function(data) {
						setTimeout(function() {
