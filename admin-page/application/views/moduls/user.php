	...
	function edit(id) {
		...
		$.ajax({
			...
			success: function(result) {
				...
				$('[name="admin_id"]').val(result.admin_id);
				$('[name="admin_username"]').val(result.admin_user);
				$('[name="admin_password"]').val(result.admin_view_password);
				$('[name="admin_hak_akses"]').val(result.admin_hak_akses);
							...
							<div class="col-xs-12">
								<form class="form-horizontal" role="form" id="formAksi">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" id="admin_username" name="admin_username" placeholder="Username" class="col-xs-10 col-sm-5" required oninvalid="this.setCustomValidity('Field username harus diisi')" oninput="this.setCustomValidity('')" />
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Password </label>
										<div class="col-sm-10">
											<input type="password" id="admin_password" name="admin_password" placeholder="Password" class="col-xs-10 col-sm-5" required oninvalid="this.setCustomValidity('Field password harus diisi')" oninput="this.setCustomValidity('')" />
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Level </label>
										<div class="col-sm-4">
											<select name="admin_hak_akses" id="admin_hak_akses" class="form-control">
												<option>--Pilih Level--</option>
												<?php $level = $this->db->get('user_type'); ?>
												<?php foreach ($level->result() as $row_kat) {	?>
