<?php $title = "<i class='fa fa-users'></i>&nbsp;Penulis"; ?>
							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover">
								<thead>
									<tr>
										<th>No.</th>
										<th>Nama Penulis</th>
										<th>Profesi Penulis</th>
										<th>Tahun Bergabung Penulis</th>
					...
					<div class="widget-main">
						<div class="row">
							<div class="col-xs-12">
								...
								<form id="form-add" class="form-horizontal" action="<?= site_url('C_penulis/ajax_add') ?>" method="POST" role="form" enctype="multipart/form-data">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" id="penulis_nama" name="penulis_nama" placeholder="Nama Penulis" class="col-xs-10 col-sm-5" required oninvalid="this.setCustomValidity('Field nama harus diisi')" oninput="this.setCustomValidity('')"/>
											<span class="help-block"></span>
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Profesi Penulis </label>
										<div class="col-sm-10">
											<input type="text" id="penulis_profesi" name="penulis_profesi" placeholder="Profesi Penulis" class="col-xs-10 col-sm-5" required oninvalid="this.setCustomValidity('Field profesi harus diisi')" oninput="this.setCustomValidity('')"/> <span class="help-block"></span>
										</div>
									</div>
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Deskripsi Penulis </label>
										<div class="col-sm-6">
											<textarea class="form-control" id="penulis_deskripsi" name="penulis_deskripsi" placeholder="Deskripsi Penulis" class="col-xs-10 col-sm-5" cols="30" rows="10"></textarea>
											<span class="help-block"></span>
										</div>
									</div>
									<!-- <div class="form-group">
				...
				<div id="home" class="tab-pane fade in active">
					<form action="#" id="form" name="form" class="form-horizontal" enctype="multipart/form-data">
						...
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Profesi Penulis </label>
							<div class="col-sm-10">
								<input type="text" id="penulis_profesi" name="penulis_profesi" placeholder=" Profesi Penulis" class="col-xs-10 col-sm-5" required oninvalid="this.setCustomValidity('Field profesi harus diisi')" oninput="this.setCustomValidity('')"></input>
								<span class="help-block"></span>
							</div>
						</div>
						<div class="form-group">
...
<script>
	var zonk = '';
	var save_method;
	var link = "<?php echo site_url('C_penulis') ?>";
	var table;
	...
	function penulis() {
		...
		table = $('#dynamic-table').DataTable({
			...
			"ajax": {
				"url": "<?php echo site_url('C_penulis/ajax_list') ?>",
				"type": "POST"
			},
	...
	function update(id) {
		...
		$.ajax({
			...
			success: function(result) {
				...
				$('[name="penulis_nama"]').val(result.penulis_nama);
				$('[name="penulis_profesi"]').val(result.penulis_profesi);
				$('[name="penulis_deskripsi"]').val(result.penulis_deskripsi);
			},
			error: function(jqXHR, textStatus, errorThrown) {
	...
	function hapus(id) {
		if (confirm('Are you sure delete this data?')) {
			$.ajax({
				url: "<?php echo site_url('C_penulis/ajax_delete') ?>/" + id,
				type: "POST",
				dataType: "JSON",
				success: function(data) {
					setTimeout(function() {
