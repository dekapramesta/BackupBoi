<?php $title = "<i class='fa fa-credit-card'></i>&nbsp;Artikel"; ?>
...
<div class="row">
	<div class="col-xs-12">
		<div id="form-data" style="display:none;">
			<div class="widget-box">
				<div class="widget-header">
					<h4 class="widget-title">Form Artikel</h4>
					...
					<div class="widget-main">
						<div class="row">
							<div class="col-xs-12">
								...
								<form id="form-add" class="form-horizontal" action="<?= site_url('C_berita/ajax_add') ?>" method="POST" role="form" enctype="multipart/form-data">
									<input type="hidden" name="berita_id">
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Judul </label>
										<div class="col-sm-10">
											<input type="text" id="berita_judul" name="berita_judul" placeholder="Judul" class="col-xs-10 col-sm-5" required oninvalid="this.setCustomValidity('Field judul harus diisi')" oninput="this.setCustomValidity('')"/>
									...
									<div class="form-group">
										...
										<div class="col-sm-6">
											<textarea class="form-control" id="berita_deskripsi" name="berita_deskripsi" placeholder="Deskripsi" class="col-xs-10 col-sm-5" cols="30" rows="10"></textarea>
											<span class="help-block"></span>
										</div>
									</div>
									<!-- <div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Artikel Penulis </label>
										<div class="col-sm-10">
											<input type="text" id="berita_autor" name="berita_autor" placeholder="Artikel Penulis" class="col-xs-10 col-sm-5" />
											<span class="help-block"></span>
										</div>
									</div> -->
									<div class="form-group">
										...
										<div class="col-sm-10">
											<select class="ui search-dropdown" style="width: 300px;" id="penulis_id" name="penulis_id">
												<?php foreach ($penulis_berita as $penulis) : ?>
									...
									<div class="form-group">
										<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Artikel Tag </label>
										<div class="col-sm-6">
											<input type="text" id="berita_tag" name="berita_tag" placeholder="Artikel Tag" class="col-xs-10 col-sm-5" required oninvalid="this.setCustomValidity('Field tag harus diisi')" oninput="this.setCustomValidity('')"/>
											<span class="help-block"></span>
										</div>
									</div>
									<div class="form-group">
		...
		<div class="tabbable">
			...
			<div class="tab-content">
				<div id="home" class="tab-pane fade in active">
					<form action="#" id="form" name="form" class="form-horizontal" enctype="multipart/form-data">
						...
						<div class="form-group">
							<label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Deskripsi </label>
							<div class="col-sm-6">
								<textarea class="form-control" id="teksarea" name="berita_deskripsi" placeholder="Deskripsi" class="col-xs-10 col-sm-5" cols="30" rows="10"></textarea>
								<span class="help-block"></span>
							</div>
						</div>
						<!-- <div class="form-group">
				...
				<div id="messages" class="tab-pane fade">
					<form id="form-upload" class="form-horizontal" role="form" action="<?= site_url('C_berita/upload') ?>" method="POST" enctype="multipart/form-data">
						<input type="hidden" name="berita_id" />
						<div class="form-body">
							<div class="form-group">
	...
	<script>
		var zonk = '';
		var save_method;
		var link = "<?php echo site_url('C_berita') ?>";
		var table;
		...
		function berita() {
			...
			table = $('#dynamic-table').DataTable({
				...
				"ajax": {
					"url": "<?php echo site_url('C_berita/ajax_list') ?>",
					"type": "POST"
				},
		...
		function berita_en() {
			...
			table = $('#dynamic-table').DataTable({
				...
				// Load data for the table's content from an Ajax source
				"ajax": {
					"url": "<?php echo site_url('C_berita/ajax_list_en') ?>",
					"type": "POST"
				},
		...
		function update(id) {
			...
			$.ajax({
				...
				success: function(result) {
					var img = '<?= base_url(); ?>../uploads/berita/' + result.berita_foto;
					$('[name="berita_id"]').val(result.berita_id);
					$('[name="berita_judul"]').val(result.berita_judul);
					tinymce.editors[1].setContent(result.berita_deskripsi);
					$('[name="berita_tag"]').val(result.berita_tag);
		...
		function hapus(id) {
			if (confirm('Are you sure delete this data?')) {
				$.ajax({
					url: "<?php echo site_url('C_berita/ajax_delete') ?>/" + id,
					type: "POST",
					dataType: "JSON",
					success: function(data) {
						setTimeout(function() {
