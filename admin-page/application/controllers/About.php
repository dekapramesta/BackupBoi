...
class About extends CI_Controller {
	...
	public function ajax_list() {
		...
		foreach ($list as $about) {
			...
			$row1 = '<img src="'.base_url().'../uploads/about/'.$cover.'" style="height: 100px; width: 100px;">';
			$no++;
			$row = array();
			$row[] = $no;
			$row[] = $row1;
	...
	public function ajax_edit_en() {
		$data = $this->Mdl_about->get_by_id_en();
		echo json_encode($data);
	}
...
// 		echo json_encode(array("status" => TRUE));
	...
	public function upload() {
		...
		$this->Mdl_about->update_data($where,$data,'about');	
